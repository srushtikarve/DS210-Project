...
pub fn bfs_dist(graph: &Vec<Vec<usize>>) -> HashMap<usize, Vec<usize>> { //looks at which nodes have connections to which ones
    ...
    for i in 0..graph.len() { //iterates over each node in the graph and performs breadth-first search
        ...
        let mut distance1 = Vec::new(); //stores non-zero distances in the distance vector
    ...
    return distance_map; //BFS determines the distances between nodes
}